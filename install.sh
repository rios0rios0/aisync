#!/bin/sh
#
# aisync Installation Script
#
# Downloads and installs aisync from GitHub releases.
# Automatically detects your operating system and architecture.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/rios0rios0/aisync/main/install.sh | sh
#   wget -qO- https://raw.githubusercontent.com/rios0rios0/aisync/main/install.sh | sh
#
# Options:
...
#   AISYNC_INSTALL_DIR   Installation directory (default: ~/.local/bin)
#   AISYNC_VERSION       Specific version to install (default: latest)
#   AISYNC_FORCE         Force installation (true/false, default: false)
#   AISYNC_DRY_RUN       Dry run mode (true/false, default: false)
#
...
INSTALL_DIR="${AISYNC_INSTALL_DIR:-$DEFAULT_INSTALL_DIR}"
VERSION="${AISYNC_VERSION:-latest}"
FORCE="${AISYNC_FORCE:-false}"
DRY_RUN="${AISYNC_DRY_RUN:-false}"
...
ENVIRONMENT VARIABLES:
    AISYNC_INSTALL_DIR   Installation directory
    AISYNC_VERSION       Specific version to install (default: latest)
    AISYNC_FORCE         Force installation (true/false)
    AISYNC_DRY_RUN       Dry run mode (true/false)
...
main() {
    ...
    info "  ${BINARY_NAME} init"
    info "  ${BINARY_NAME} source add guide --source-repo rios0rios0/guide --branch generated"
    info "  ${BINARY_NAME} pull"
}
