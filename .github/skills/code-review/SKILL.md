---
name: code-review
description: "Review pull requests and diffs in aisync — the Go CLI that syncs AI-assistant configurations across devices — against the rios0rios0/guide standards: the manual composition root, the domain/infrastructure boundary, hand-rolled test doubles, age-encryption safety, and chlog fragments. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `aisync`

`aisync` is a Go CLI that syncs AI coding-assistant configurations — rules, agents, commands, hooks, skills, memories — across devices, with age encryption and multi-source support. It is Clean/Hexagonal Go wired by hand, with no DI framework, and it writes to the user's real home directory, so a review here weighs data-destruction risk as heavily as style.
...
## What matters most in `aisync`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

```
cmd/aisync/                 entry point — logrus setup, controllers.NewRootCommand(version)
internal/domain/            contracts, zero framework dependencies
  commands/                   use cases, single Execute()
  entities/                   value objects and aggregates
  repositories/               ports
internal/infrastructure/    adapters
  controllers/                cobra wiring + the manual composition root (root.go)
  repositories/               YAML / JSON / HTTP / Git persistence
  services/                   encryption, mergers, watchers, scanners
  ui/                         lipgloss formatting
test/doubles/               hand-written struct stubs for every domain interface
```

- **`internal/infrastructure/controllers/root.go` is the only composition root.** It is the sole place a concrete infrastructure type may be named. A domain command constructor that accepts a concrete adapter instead of an interface is a Critical finding.
- **No DI framework.** Manual constructor wiring is a deliberate choice — reject any pull request that introduces Uber Dig, Wire, or similar here.
...
- Test files live in an external `_test` package (`package commands_test`) and touch only exported API.
- Names follow `TestTypeName_MethodName_DescriptiveBehaviour`, with each scenario in a `t.Run("should … when …", …)` subtest.
- `t.Parallel()` sits at the top of the parent `Test…` function; a test that mutates process-wide state carries a comment saying why it is not parallel.
- Assertions use `testify/assert`; fatal preconditions use `testify/require`.
- Doubles are the hand-written struct stubs in `test/doubles/mocks.go`, which record call counts and captured arguments. A new domain interface means a new stub there in the same style.
...
- The manual DI wiring in `root.go` being verbose — that verbosity is the point.
- Generated coverage artefacts (`cobertura.xml`, `coverage.txt`, `coverage.xml`, `junit.xml`, `reports/`) appearing in the tree; they are build output, not source. Do flag them if a commit actually adds them to git.
