...
func TestExecGitInspector_LocalRemotes(t *testing.T) {
	...
	t.Run("should parse an ADO SSH remote with -alias suffix", func(t *testing.T) {
		...
		// given — `git@ssh.dev.azure.com-northwind:v3/...` is the form
		// that emerges when the user has an SSH host alias for ADO
		// (e.g. multiple ADO accounts on the same machine).
		requireGit(t)
		root := initGitRepoWithRemote(t, "git@ssh.dev.azure.com-northwind:v3/AcmeOrg/AcmeProject/some-repo")
...
`
		...
		require.NoError(t, err)
		values := derivedTermValues(terms)
		assert.Contains(t, values, "northwind")
...
func TestExecGitInspector_EmailDomain(t *testing.T) {
	...
	t.Run("should return the domain for an employer email", func(t *testing.T) {
		// given
		requireGit(t)
		writeGlobalGitConfig(t, "alice@northwind.ca")
		inspector, err := repositories.NewExecGitInspector()
		require.NoError(t, err)
		...
		// then
		require.NoError(t, err)
		assert.Equal(t, "northwind.ca", domain)
	})
