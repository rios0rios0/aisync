...
func TestAgeForbiddenTermsRepository(t *testing.T) {
	...
	t.Run("should round-trip terms through encrypt+decrypt", func(t *testing.T) {
		...
		canonical, err := entities.NewCanonicalTerm("ContosoSecurity", "user")
		require.NoError(t, err)
		word, err := entities.NewCanonicalWordTerm("QA", "user")
		require.NoError(t, err)
		regexTerm, err := entities.NewRegexTerm(`\bContoso-[A-Z]\w+\b`, "user")
		require.NoError(t, err)
		...
		assert.Equal(t, "ContosoSecurity", loaded[0].Original)
		...
		assert.Equal(t, `\bContoso-[A-Z]\w+\b`, loaded[2].Original)
