...
func TestAutoDeriver_DeriveTerms(t *testing.T) {
	...
	t.Run("should aggregate findings from every source", func(t *testing.T) {
		...
		inspector := &doubles.MockGitInspector{
			EmailDomainVal: "northwind.ca",
			LocalRemotesVal: []repositories.DerivedTerm{
				{Value: "ContosoSecurity", Origin: "git-remote:dev.azure.com"},
			},
			DirectoryLayoutVal: []repositories.DerivedTerm{
				{Value: "Contoso-App", Origin: "fs:~/Development/dev.azure.com/ContosoSecurity"},
			},
			SSHHostAliasesVal: []repositories.DerivedTerm{
				{Value: "northwind", Origin: "ssh-config:Host:dev.azure.com"},
			},
		}
	...
	t.Run("should filter self-identities from derived terms", func(t *testing.T) {
		...
		assert.Equal(t, "ContosoSecurity", terms[0].Original)
	})
	...
	t.Run("should dedupe across sources via canonical form", func(t *testing.T) {
		...
		inspector := &doubles.MockGitInspector{
			...
			DirectoryLayoutVal: []repositories.DerivedTerm{
				// Same canonical form (`contososecurity`) as the git remote
				// owner above — dedupe must collapse them.
				{Value: "Contoso-Security", Origin: "fs:~/Development/dev.azure.com"},
			},
		}
		deriver := newAutoDeriverWithTempCache(t, inspector)
		...
		assert.Equal(t, "contososecurity", entities.Canonicalize(terms[0].Original))
	})
	...
	t.Run("should drop terms matching the compile-time generic-name stop list", func(t *testing.T) {
		...
		// `back.end`), and ONE real org name (`ContosoSecurity`). Only the
